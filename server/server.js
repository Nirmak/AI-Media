...
app.get('/api/creative-rewrite', async (req, res) => {
  ...
  if (!currentPdfName || !bookService.getAnalysisResults(currentPdfName)) {
  ...
  const analysisResults = bookService.getAnalysisResults(currentPdfName);
  ...
  prompt += `**Source Book:** ${pdfTitle || currentPdfName}\n\n`;
